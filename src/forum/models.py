...
class Conversation(BaseModel):
    title = models.CharField(
        max_length=100, help_text=_("The maximum number of characters is 100.")
    )
    author = models.ForeignKey(
        get_user_model(), default="Unknown", on_delete=SET_DEFAULT
    )
    content = models.TextField(
        blank=True,
        max_length=11000,
        help_text=_("The maximum number of characters is 11000."),
    )
    image = models.ImageField(
        upload_to="forum/conversation/images", null=True, blank=True
    )
...
class Comments(BaseModel):
    ...
    content = models.TextField(
        blank=True,
        max_length=8000,
        help_text=_("The maximum number of characters is 8000."),
    )
